...
public class RPCAddressParser implements RemotingAddressParser {
    ...
    public void initUrlArgs(ConnectionURL connectionURL) {
        ...
        String connTimeoutStr = connectionURL.getProperty(RPCConfigsSupport.CONNECT_TIMEOUT_KEY);
        if (StringUtils.isNotBlank(connTimeoutStr)) {
            if (StringUtils.isNumeric(connTimeoutStr)) {
                ...
                throw new IllegalArgumentException("Url args illegal value of key [" + RPCConfigsSupport.CONNECT_TIMEOUT_KEY + "] must be positive integer! The origin url is [" + connectionURL.getOriginUrl() + "]");
            }
        }
        ...
        String protocolStr = connectionURL.getProperty(RPCConfigsSupport.URL_PROTOCOL);
        if (StringUtils.isNotBlank(protocolStr)) {
            if (StringUtils.isNumeric(protocolStr)) {
                protocol = Byte.parseByte(protocolStr);
            } else {
                throw new IllegalArgumentException("Url args illegal value of key [" + RPCConfigsSupport.URL_PROTOCOL + "] must be positive integer! The origin url is [" + connectionURL.getOriginUrl() + "]");
            }
        }
        ...
        String versionStr = connectionURL.getProperty(RPCConfigsSupport.URL_VERSION);
        if (StringUtils.isNotBlank(versionStr)) {
            if (StringUtils.isNumeric(versionStr)) {
                version = Byte.parseByte(versionStr);
            } else {
                throw new IllegalArgumentException("Url args illegal value of key [" + RPCConfigsSupport.URL_VERSION + "] must be positive integer! The origin url is [" + connectionURL.getOriginUrl() + "]");
            }
        }
        ...
        String connNumStr = connectionURL.getProperty(RPCConfigsSupport.CONNECTION_NUM_KEY);
        if (StringUtils.isNotBlank(connNumStr)) {
            if (StringUtils.isNumeric(connNumStr)) {
                connNum = Integer.parseInt(connNumStr);
            } else {
                throw new IllegalArgumentException("Url args illegal value of key [" + RPCConfigsSupport.CONNECTION_NUM_KEY + "] must be positive integer! The origin url is [" + connectionURL.getOriginUrl() + "]");
            }
        }
        ...
        String connWarmupStr = connectionURL.getProperty(RPCConfigsSupport.CONNECTION_WARMUP_KEY);
