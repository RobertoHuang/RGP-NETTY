...
public class RPCClientRemoting extends RPCRemoting {
    ...
    protected void preProcessInvokeContext(Connection connection, RemotingCommand remotingCommand, InvokeContext invokeContext) {
        if (null != invokeContext) {
            ...
            invokeContext.putIfAbsent(InvokeContext.CLIENT_LOCAL_IP, RemotingAddressUtil.parseLocalIP(connection.getChannel()));
            invokeContext.putIfAbsent(InvokeContext.CLIENT_LOCAL_PORT, RemotingAddressUtil.parseLocalPort(connection.getChannel()));
            invokeContext.putIfAbsent(InvokeContext.CLIENT_REMOTE_IP, RemotingAddressUtil.parseRemoteIP(connection.getChannel()));
            invokeContext.putIfAbsent(InvokeContext.CLIENT_REMOTE_PORT, RemotingAddressUtil.parseRemotePort(connection.getChannel()));
        }
    }
